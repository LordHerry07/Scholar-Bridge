...
# ==========================================
# DOMAIN: CORE CONFIG & CACHING
# ==========================================

# -------------------------------
# SETUP: Builder Initialization
# -------------------------------
...
# ==========================================
# DOMAIN: MODALS & OVERLAYS
# ==========================================

# -------------------------------
# OVERLAY: Visual Loaders
# -------------------------------
...
class NotificationModal(ModalView):
    def show(self, title, message, is_error=False):
        ...
        if is_error:
            self.ids.notif_title.color = (0.88, 0.25, 0.25, 1) 
        else:
            self.ids.notif_title.color = (0.16, 0.67, 0.38, 1) 
        self.ids.notif_message.text = message
        self.open()

# -------------------------------
# MODAL: Authentication Tools
# -------------------------------
...
# -------------------------------
# MODAL: Marketplace Transactions
# -------------------------------
class ReviewModal(ModalView):
    seller_name = StringProperty("")
    current_rating = NumericProperty(0) 
    def submit(self):
...
class EditProfileModal(ModalView):
    ...
    def auto_locate(self):
        if hasattr(self.ids, 'location_input'):
            ...
            self.ids.location_input.ids.internal_input.foreground_color = (0.43, 0.33, 0.85, 1) 
...
# ==========================================
# DOMAIN: DYNAMIC COMPONENTS (TILES & CARDS)
# ==========================================

# -------------------------------
# COMPONENTS: Dashboard Items
# -------------------------------
...
# ==========================================
# DOMAIN: MAIN SCREENS (SCREENS & HUBS)
# ==========================================

# -------------------------------
# SCREEN: App Navigation Root
# -------------------------------
...
class MainInterface(Screen):
    ...
    def switch_tab(self, tab_name):
        ...
        if current_tab in tab_order and tab_name in tab_order:
            ...
            if target_idx > current_idx: sm.transition.direction = 'left'
            elif target_idx < current_idx: sm.transition.direction = 'right'
...
class StartInterface(Screen):
    ...
    def switch_tab(self, tab_name):
        sm = self.ids.screenmanager
        if tab_name == 'login': sm.transition.direction = 'right'
        else: sm.transition.direction = 'left'
        sm.current = tab_name
    ...
    def _process_login(self, response):
        ...
        if response and response.status_code == 200:
            ...
            try:
                with open('local_state.json', 'w', encoding='utf-8') as f:
                    json.dump(Data.user, f)
            except Exception as e: print("Failed to save session:", e)
            app = App.get_running_app()
            app.root.logged = True
            app.root.current = "main"
        else:
...
class Signup(Widget):
    ...
    def sign_user(self, full_name, email, password, confirm_password):
        ...
        if "@" not in email or not email.endswith(".edu.ph"):
            NotificationModal().show("Invalid Email", "You must use a valid Philippine university email ending in '.edu.ph'.", is_error=True)
            return
        if len(password) <= 7:
...
class Dashboard(Widget):
    ...
    def load_stats(self, dt=0):
        ...
        if stats:
            ...
            if hasattr(self.ids, 'earnings_trend'): self.ids.earnings_trend.text = f"[color=#{e_color}]{e_sign}{e_pct}%[/color]"
            if hasattr(self.ids, 'listings_trend'): self.ids.listings_trend.text = f"[color=#{l_color}]{l_sign}{l_pct}%[/color]"
            if hasattr(self.ids, 'sold_trend'): self.ids.sold_trend.text = f"[color=#{s_color}]{s_sign}{s_pct}%[/color]"
            if hasattr(self.ids, 'rating_trend'): self.ids.rating_trend.text = f"[b]{stats.get('top_percentage', 'Top 1%')}[/b]"

            graph_data = stats.get('graph_data', [])
            while len(graph_data) < self.MAX_BARS: graph_data.insert(0, 0)
    ...
    def draw_graph(self):
        ...
        with chart.canvas.after:
            for i, v in enumerate(self.data):
                if i == len(self.data) - 1 and v > 0: Color(0.16, 0.67, 0.38, 1) 
                elif i == len(self.data) - 1: Color(0.2, 0.7, 0.9, 1) 
                elif i == 0: Color(0.6, 0.6, 0.6, 1)  
                else:
                    prev = self.data[i - 1]
                    if v > prev: Color(0.3, 0.9, 0.3, 1)  
                    elif v < prev: Color(0.9, 0.3, 0.3, 1)  
...
class Product(Widget):
    ...
    def _thread_fetch_products(self):
        ...
        try:
            ...
            if response: save_to_cache('marketplace_products', response)
        except Exception as e:
...
class Sell(Widget):
    ...
    def add_product(self):
        ...
        if not self.is_service_mode:
            ...
            payload = {
                "initial": initial, "full_name": full_name, "title": title,
                "subject": subject_name, "product_type": listing_type, 
            ...
            payload = {
                "initial": initial, "full_name": full_name, "title": title,
                "subject": subject_name, "category": category, "rate": price,
...
class Service(Widget):
    ...
    def _on_services_fetched(self, response):
        if response: self.all_services = response 
        self.filter_services()
    ...
    def load_hub_data(self, dt=0):
        ...
        if self.current_tab == 'chats':
            ...
            for sub in hub_data.get('subscriptions', []):
                ...
                tile.seller_name = sub.get('full_name', 'Unknown Seller') 
                container.add_widget(tile)
...
class Wallet(BoxLayout):
    ...
    def process_transaction(self, action):
        ...
        if not my_email: return
        if not amount_text:
            NotificationModal().show("Action Blocked", "Please enter an amount first.", is_error=True)
            return
        try:
        ...
        if success:
            ...
            self.load_balance() 
            NotificationModal().show("Transaction Successful", f"Successfully {action}ed ₱{amount:,.2f}.", is_error=False)
        else:
            if action == 'withdraw':
                NotificationModal().show("Transaction Failed", "Insufficient funds or network error.", is_error=True)
            else:
                NotificationModal().show("Transaction Failed", "Could not process your request.", is_error=True)
...
class SettingsInterface(Screen):
    ...
    def logout_user(self):
        ...
        if os.path.exists('local_state.json'):
            try: os.remove('local_state.json')
            except: pass
        app = App.get_running_app()
